...
Journal.write("################################## System Start:" + Get_time_str(':') + " ###########################################\n")
Journal.close()
...
def status_api(): # Socket communicate with other process and web server.
    print "Try to use the other usart channle to communicate with Arduino and create a interrupt to deal with the CMD."
    print "Listen to the AWS CMD.Get the global value of sensor and state of Delay-Controller and Update the state to AWS by boto3 lib."
    print "Listen to the AWS CMD.Set the global value of sensor and state of Delay-Controller(mannully or automatic)"
    print "Listen to the Local web page request,show the basic info."
    print "Listen to the Local web page request,set the basic state."
...
while True:
    ...
    if pre_min != date.tm_min:
        ...
        if date.tm_hour >= 8 and date.tm_hour < 19: # Judge the CO2 detect time.
            print "Monitor CO2..."
            Time_Point = Get_time_str('-',struct=date) # ADD-line
    ...
    if date.tm_hour % 1 == 0 and date.tm_min % 59 == 0 and min_update:
        ...
        Time_NOW = Get_time_str('-',struct=date) # ADD-line
            ...
            for y in range(y_n):
                ...
                while Camera_Status == 'BUSY':
                    ...
                    try:
                        ...
                        print 'Camera Wrong!'
                        Camera_Status = 'BUSY'
                        Error_tm_str = Get_time_str(':')
                        Journal = open(abs_path,'a')
                        Journal.write(Error_tm_str + "-Camera Wrong? "+str(Exception)+':'+str(e)+'\n')
                        Journal.close()
                        time.sleep(5)
                        pass
                Journal = open(abs_path,'a')
